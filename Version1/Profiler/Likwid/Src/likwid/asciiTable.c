...
asciiTable_allocate(int numRows,int numColumns, bstrList* headerLabels)
{
    ...
    {
        ERROR_MSG(Number of columns not equal to number of header labels);
    }
...
asciiTable_free(TableContainer* container)
{
    ...
    if(container == NULL)
    {
        ERROR_MSG(Cannot free NULL reference);
    }
...
asciiTable_insertRow(TableContainer* container, int row, bstrList* fields)
{
    ...
    {
        ERROR_MSG(Number of colummns not equal to number of field labels);
    }

    if (row >= container->numRows)
    {
        ERROR_MSG(asciiTable: Number of Rows smaller than requested row index);
    }
