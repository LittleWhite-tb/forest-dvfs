...
#if 0
static void
cacheDescriptorTable(int value, CacheLevel* levelData)
{
    switch( value )
    {
        case 0x00:
        ...
        case 0x06:
            /*
            levelData->level = 1;
            levelData->type = INSTRUCTIONCACHE;
            levelData->associativity = 4;
            levelData->sets = 64;
            levelData->lineSize = 32;
            levelData->size = 8192;
            levelData->threads = 1;
            levelData->inclusive = 0;
            */
            break;
        case 0x08:
            /*
        ...
        case 0x0A:
            /*
            levelData->level = 1;
            levelData->type = DATACACHE;
            levelData->associativity = 2;
            levelData->sets = 64;
            levelData->lineSize = 32;
            levelData->size = 8192;
            levelData->threads = 1;
	...
	for (i=0; i < maxNumLevels; i++) 
	{
		...
		if (i < 3)
		{
			if ((cpuid_info.model == NEHALEM_BLOOMFIELD) ||
					...
					(cpuid_info.model == XEON_SANDYBRIDGE) ||
