...
void affinity_init()
{
    ...
    numberOfCacheDomains = cpuid_topology.numSockets * (cpuid_topology.numCoresPerSocket/numberOfCoresPerCache);
    ...
    for (i=1; i<numberOfDomains; i++) {
        ...
        else if (i < (int) ((cpuid_topology.numSockets + numberOfCacheDomains)+1))
        {
            ...
            if (i < (int) ((cpuid_topology.numSockets + numberOfCacheDomains)+1)) currentDomain = 0;
        }
        else 
        {
            domains[i].processorList = (int*)
