...
PapiCounters::PapiCounters (void)
{
	...
	if (PAPI_event_name_to_code ("SQ_FULL_STALL_CYCLES", &ev_codes[0]) != PAPI_OK) 
	{
        std::cerr << "PAPI even_name_to_code failed!" << std::endl;
		exit (1);
	}

	if (PAPI_event_name_to_code ("UNHALTED_CORE_CYCLES", &ev_codes[1]) != PAPI_OK) 
	{
        std::cerr << "PAPI even_name_to_code failed!" << std::endl;
		exit (1);
	}

	if (PAPI_event_name_to_code ("L2_RQSTS:MISS", &ev_codes[2]) != PAPI_OK) 
	{
        std::cerr << "PAPI even_name_to_code failed!" << std::endl;
		exit (1);
	}
