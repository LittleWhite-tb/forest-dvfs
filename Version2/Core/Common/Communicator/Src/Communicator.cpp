...
Communicator::Communicator ()
{
   ...
   // we do not want to receive any SIGPIPE in case of error on sockets
   ...
   while (true)
   {
      ...
      if (sres > 0)  // some fd has received data
      {
               ...
               it_sockin++)
         {
            ...
            if (msg == NULL)
            {
               this->disconnect (it_sockin->first);
            }
            else
            {
...
void Communicator::disconnect (unsigned int id)
{
   std::map<CommConnectFn, void *>::iterator it_fn;
   std::map<unsigned int, int>::iterator it_sock;


   std::cout << "Connection with node " << id << " lost" << std::endl;

   // close incoming connection
   it_sock = this->sockets_in.find (id);

   if (it_sock != this->sockets_in.end ())
   {
      close (this->sockets_in [id]);
      this->sockets_in.erase (it_sock);
   }

   // same for outgoing connections
   it_sock = this->sockets_out.find (id);

   if (it_sock != this->sockets_out.end ())
   {
      close (this->sockets_out [id]);
      this->sockets_out.erase (it_sock);
   }
   ...
   for (it_fn = this->connCallbacks->begin ();
         ...
         it_fn++)
   {
      it_fn->first (false, id, it_fn->second);
   }
}
